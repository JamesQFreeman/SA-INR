...
class MakeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gradient=sitk.GetArrayFromImage(sitk.ReadImage(self.path_list[idx].replace("xr","gradient")))
        gradient=gradient.T
        
        mask=self.get_mask(gradient)
        ...
        sp=torch.FloatTensor([1])-torch.nonzero(crop_lr).shape[0]/self.inp_num 
        #print(crop_lr.shape)
